...
def findPatchHorizontal(refBlock, texture, blocksize, overlap, tolerance):
    ...
    for i, j in product(range(H - blocksize), range(W - blocksize)):
        rmsVal = (
            (texture[i : i + blocksize, j : j + overlap] - refBlock[:, -overlap:]) ** 2
        ).mean()
        if rmsVal > 0:
            errMat[i, j] = rmsVal
...
def findPatchBoth(refBlockLeft, refBlockTop, texture, blocksize, overlap, tolerance):
    ...
    for i, j in product(range(H - blocksize), range(W - blocksize)):
        rmsVal = (
            (texture[i : i + overlap, j : j + blocksize] - refBlockTop[-overlap:, :])
            ** 2
        ).mean()
        rmsVal = (
            rmsVal
            + (
                (
                    texture[i : i + blocksize, j : j + overlap]
                    - refBlockLeft[:, -overlap:]
                )
                ** 2
            ).mean()
        )
...
def findPatchVertical(refBlock, texture, blocksize, overlap, tolerance):
    ...
    for i, j in product(range(H - blocksize), range(W - blocksize)):
        rmsVal = (
            (texture[i : i + overlap, j : j + blocksize] - refBlock[-overlap:, :]) ** 2
        ).mean()
        if rmsVal > 0:
            errMat[i, j] = rmsVal
...
def getMinCutPatchVertical(block1, block2, blocksize, overlap):
    ...
    """
    resBlock = getMinCutPatchHorizontal(
        np.rot90(block1), np.rot90(block2), blocksize, overlap
    )
...
def getMinCutPatchBoth(refBlockLeft, refBlockTop, patchBlock, blocksize, overlap):
    ...
    err = (
        (np.rot90(refBlockTop)[:, -overlap:] - np.rot90(patchBlock)[:, :overlap]) ** 2
    ).mean(2)
    # maintain minIndex for 2nd row onwards and
    minIndex = []
    E = [list(err[0])]
    ...
    mask2[:overlap, :overlap] = np.maximum(
        mask2[:overlap, :overlap] - mask1[:overlap, :overlap], 0
    )
    ...
    resBlock[:overlap, :] = (
        resBlock[:overlap, :] + mask2[:overlap, :] * refBlockTop[-overlap:, :]
    )
...
def generateTextureMap(image, blocksize, overlap, outH, outW, tolerance):
    ...
    textureMap = np.zeros(
        (
            (blocksize + nH * (blocksize - overlap)),
            (blocksize + nW * (blocksize - overlap)),
            image.shape[2],
        )
    )
    ...
    # Fill the first row
    print("Filling the first row...")
    for i, blkIdx in tqdm(
        enumerate(
            range(
                (blocksize - overlap),
                textureMap.shape[1] - overlap,
                (blocksize - overlap),
            )
        )
    ):
        ...
        refBlock = textureMap[
            :blocksize, (blkIdx - blocksize + overlap) : (blkIdx + overlap)
        ]
    ...
    print("Filling the first column...")
    for i, blkIdx in tqdm(
        enumerate(
            range(
                (blocksize - overlap),
                textureMap.shape[0] - overlap,
                (blocksize - overlap),
            )
        )
    ):
        ...
        # blkIdx = block index to put in
        refBlock = textureMap[
            (blkIdx - blocksize + overlap) : (blkIdx + overlap), :blocksize
        ]
    ...
    print("Filling the other rows and columns...")
    for i in tqdm(range(1, nH + 1)):
        for j in range(1, nW + 1):
            ...
            refBlockLeft = textureMap[
                (blkIndexI) : (blkIndexI + blocksize),
                (blkIndexJ - blocksize + overlap) : (blkIndexJ + overlap),
            ]
            refBlockTop = textureMap[
                (blkIndexI - blocksize + overlap) : (blkIndexI + overlap),
                (blkIndexJ) : (blkIndexJ + blocksize),
            ]

            patchBlock = findPatchBoth(
                refBlockLeft, refBlockTop, image, blocksize, overlap, tolerance
            )
            minCutPatch = getMinCutPatchBoth(
                refBlockLeft, refBlockTop, patchBlock, blocksize, overlap
            )
